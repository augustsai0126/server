					     ...
					     * by os_thread_create */
{
	...
	static std::set<uint>	completed_threads_list;
	mutex_enter(&fil_crypt_threads_mutex);
	...
	while (!thr.should_shutdown()) {
		...
		while (!thr.should_shutdown() &&
			...
			if (completed_threads_list.find(thr.thread_no)
			    != completed_threads_list.end()) {
				completed_threads_list.erase(thr.thread_no);
			}
		...
		if (!thr.should_shutdown() && !recheck) {

			mutex_enter(&fil_crypt_threads_mutex);

			completed_threads_list.insert(thr.thread_no);

			/* All crypt threads processed all existing tablespace. */
			if ((completed_threads_list.size()
				== srv_n_fil_crypt_threads_started)
			    && !fil_crypt_valid_state()) {
